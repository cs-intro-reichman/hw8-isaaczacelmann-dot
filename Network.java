...
public class Network {
    ...
    public String recommendWhoToFollow(String name) {
        String maxMutualName = null;
        int maxMutualCount = 0;
        for (int i = 0; i < userCount; i++) {
            if (users[i].getName() != name) {
                int mututalCount = users[i].countMutual(getUser(name));
                if (maxMutualCount < mututalCount) {
                    maxMutualCount = mututalCount;
    ...
    public String mostPopularUser() {
        String maxPopularName = null;
        int maxPopularCount = 0;
        for (int i = 0; i < userCount; i++) {
            int followersCount = followeeCount(users[i].getName());
            if (maxPopularCount < followersCount) {
                maxPopularCount = followersCount;
                maxPopularName = users[i].getName();
            }
        }
        return maxPopularName;
    }
    ...
    private int followeeCount(String name) {
        int count = 0;
        for (int i = 0; i < userCount; i++) {
            count += users[i].follows(name) ? 1 : 0;
        }
        return count;
    }
